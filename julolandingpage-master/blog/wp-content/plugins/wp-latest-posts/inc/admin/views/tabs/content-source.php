...
$content_type = array(
    ...
    'src_custom_post_type' => __('Custom post type', 'wp-latest-posts'),
);
?>
<script>
    (function ($) {
        $(document).ready(function () {
            ...
            $('.content-source-tab li.tab').click(function () {
                ...
                $('.wplp-source-content-selector span.content-selector').text(text);
            });
...
<div class="wplp-source-content-selector" style="margin-bottom: 20px">
    <label class="source-content-selector-label wplp-wrapper-title title-color">
        ...
        <span class="content-selector">
            <?php echo ((isset($settings['source_type']) && array_key_exists($settings['source_type'], $content_type)) ? esc_html($content_type[$settings['source_type']]) : '') ?>
        </span>
    </label>
...
<?php if (function_exists('icl_object_id') || function_exists('pll_languages_list')) : ?>
    <div class="wplp-multi-language">
        <div class="content-source-language">
            <label for="content_language" class="content-language-label wplp-wrapper-title"><?php esc_html_e('Content language', 'wp-latest-posts'); ?></label>
            <select id="content_language" class="content-language-select browser-default wplp-font-style" name="wplp_content_language">
    <!-- CHECK WPML or Polylang is INSTALLED AND ACTIVED -->
                <?php if (!empty($active_languages)) : ?>
                    <?php foreach ($active_languages as $k => $languages) :
                        ...
                        ?>
                        <option value="<?php echo esc_html($k); ?>" <?php echo esc_html($check); ?>>
                ...
                <?php elseif (!empty($poly_languages)) : ?>
                    <?php foreach ($poly_languages as $code => $languages) :
                        ...
                        ?>
                        <option value="<?php echo esc_html($code); ?>" <?php echo esc_html($check); ?>><?php echo esc_html($languages); ?></option>
                    <?php endforeach; ?>
                <?php endif; ?>
            </select>
        </div>
            ...
            if (class_exists('WPLPAddonAdmin')) :
                ...
                <li class="tab" data-id="src_custom_post_type">
                    <a href="#custom-posttype" class="link-tab">
                        <?php esc_html_e('Custom Post Type', 'wp-latest-posts') ?>
                    </a>
                </li>
            <?php endif; ?>
        </ul>
    </div>
    <input type="hidden" id="wplp_source_type" name="wplp_source_type" value="<?php echo (isset($settings['source_type']) ? esc_html($settings['source_type']) : 'src_category')?>">
    <div id="post-category" class="tab-content">
        ...
        <div class="settings-wrapper">
                ...
                <div class="multisite-select-field settings-wrapper-field">
                    ...
                    <select id="mutilsite_select_post" class="mutilsite_select wplp-font-style wplp-short-text width-30"
                        ...
                        foreach ($all_blog_cat_post as $val) {
                            ...
                            $check  = ($settings['mutilsite_cat'] === $val->blog_id) ? ' checked="checked"' : '';
            ...
            <div class="content-search-field">
                <input type="text" class="content-search-input search-input" placeholder="<?php esc_html_e('Search content', 'wp-latest-posts')?>">
                <i class="mi mi-search"></i>
            </div>
            <div class="list-selector-field settings-wrapper-field postcat">
                <ul class="craft">
                    <li><input type="checkbox" name="wplp_source_category[]" id="cat_all" value="_all" class="ju-checkbox"
    ...
    <div id="category-list" class="tab-content">
        <div class="settings-wrapper">
            <?php if (is_multisite()) :
                ...
                <div class="multisite-select-field settings-wrapper-field">
                    <label class="settings-wrapper-title"><?php esc_html_e('Multisite selection', 'wp-latest-posts') ?></label>
                    <select id="mutilsite_cat_list_select" class="mutilsite_select wplp-font-style wplp-short-text width-30"
                            name="wplp_mutilsite_cat_list">
                        <option value="all_blog"><?php esc_html_e('All blog', 'wp-latest-posts') ?></option>
                        <?php
                        foreach ($all_blog_cat_list as $val) {
                            $detail = get_blog_details((int) $val->blog_id);
                            $check  = ($settings['mutilsite_cat_list'] === $val->blog_id) ? ' checked="checked"' : '';
                            echo '<option value="' . esc_html($val->blog_id) . '" ' . esc_html($check) . '> ' . esc_html($detail->blogname) . ' </option>';
                        }
                        ?>
                    </select>
            ...
            <hr>
            <div class="sort-order-field settings-wrapper-field">
                <label class="settings-wrapper-title"><?php esc_html_e('Posts sort order', 'wp-latest-posts') ?></label>
                <ul class="un-craft">
        ...
        <div class="settings-wrapper">
                ...
                <div class="multisite-select-field settings-wrapper-field">
                    ...
                    <select id="mutilsite_select_page" class="mutilsite_select wplp-font-style wplp-short-text width-30"
                        ...
                        foreach ($all_blog_pages as $val) {
                            $detail = get_blog_details((int) $val->blog_id);
                            $check  = ($settings['mutilsite_page'] === $val->blog_id) ? ' checked="checked"' : '';
                            echo '<option value="' . esc_html($val->blog_id) . '" ' . esc_html($check) . '> ' . esc_html($detail->blogname) . ' </option>';
                        }
                        ?>
                    </select>
            ...
            <div class="list-selector-field settings-wrapper-field pagecat">
                <ul class="craft">
                    ...
                    if (!class_exists('WPLPAddonAdmin')) {
                        ?>
                        <li><input id="pages_all" type="checkbox" name="wplp_source_pages[]" class="ju-checkbox"
